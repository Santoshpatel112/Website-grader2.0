    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="text-center space-y-4 max-w-md w-full">
          <h1 className="text-xl sm:text-2xl font-bold text-muted-foreground">{t('noDataFound')}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          ...
          <div className="space-y-6 sm:space-y-8">
            <div className="mb-6 sm:mb-8">
              <h2 className="text-xl sm:text-2xl font-semibold mb-2">{t('analysisResults')}</h2>
            ...
            <Card className="hover:shadow-lg transition-shadow">
              <CardHeader className="p-4 sm:p-6">
                <CardTitle className="text-lg sm:text-xl">{t('contactMe')}</CardTitle>
              </CardHeader>
              <CardContent className="p-4 sm:p-6">
                <div className="flex justify-center space-x-6">
                  <a href="mailto:santoshpatelvns5@gmail.com" className="text-muted-foreground hover:text-primary transition-colors p-2" aria-label="Email">
                    <Mail className="w-5 h-5 sm:w-6 sm:h-6" />
                  </a>
                  <a href="tel:+918467941850" className="text-muted-foreground hover:text-primary transition-colors p-2" aria-label="Phone">
                    <Phone className="w-5 h-5 sm:w-6 sm:h-6" />
                  </a>
                  <a href="https://x.com/Santoshp3259330" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors p-2" aria-label="Twitter">
                ...
                <div className="flex items-center justify-center">
                  <div className="relative w-32 h-32 sm:w-48 sm:h-48">
                    <svg className="w-full h-full" viewBox="0 0 100 100">
                      ...
                      <circle className={`${getScoreColor(aggregate)} transition-all duration-500`} strokeWidth="8" strokeDasharray={`${aggregate * 2.51} 251.2`} strokeLinecap="round" stroke="currentColor" fill="transparent" r="40" cx="50" cy="50" />
                    </svg>
                ...
                <Card key={index} className="hover:shadow-lg transition-shadow">
                  ...
                  <CardContent className="p-4 sm:p-6">
                    <div className="space-y-3 sm:space-y-4">
                      ...
                      <Progress value={item.score} className={`h-1.5 sm:h-2 ${getScoreBgColor(item.score)}`} />
                    </div>
                  </CardContent>
                </Card>
              ))}
        ...
        <div className="container-custom px-4">
          <div className="flex flex-col items-center">
            <div className="flex items-center space-x-3 sm:space-x-4 mb-3 sm:mb-4">
              <img src="/newlogo.png" alt={t('title')} className="h-8 sm:h-12 w-auto" />
              <h1 className="text-lg sm:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-primary/80">
                {t('title')} <sup className="text-xs sm:text-sm">®</sup>
              </h1>
            </div>
            <p className="text-xs sm:text-sm text-muted-foreground">
              © {new Date().getFullYear()} {t('title')}. {t('allRights')}
            </p>
          </div>
        </div>
      </footer>
